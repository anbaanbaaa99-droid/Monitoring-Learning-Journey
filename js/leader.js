...
const LEADER_SESSION_KEY = "ppm_leader_session_v1";
...
const LEADER_FRONTEND_VERSION = "20260829-hrfinal1";
...
function initializeLeaderPage() {
  ...
  if (missing.length) {
    console.error("Leader UI tidak lengkap:", missing);
    document.body.insertAdjacentHTML(
      "afterbegin",
      `<div style="position:fixed;z-index:99999;left:12px;right:12px;top:12px;padding:12px;background:#fff0f3;border:1px solid #c43a55;border-radius:8px;color:#8f1f35;font:14px Arial">Frontend Leader tidak lengkap. Elemen hilang: ${missing.join(", ")}</div>`
    );
    return;
  }

  loginFormEl.addEventListener("submit", handleLeaderLogin);
  togglePasswordEl?.addEventListener("click", togglePasswordVisibility);
  logoutEl?.addEventListener("click", logoutLeader);

  window.addEventListener("error", event => {
    console.error("Leader frontend error:", event.error || event.message);
    if (!hrAppEl.hidden) return;
    setLoginStatus(`JavaScript error: ${event.message || "unknown error"}`, true);
  });
  window.addEventListener("unhandledrejection", event => {
    console.error("Leader unhandled rejection:", event.reason);
    if (!hrAppEl.hidden) return;
    const msg = event.reason?.message || String(event.reason || "unknown error");
    setLoginStatus(`Error: ${msg}`, true);
  });

  console.info(`Leader frontend ${LEADER_FRONTEND_VERSION} loaded`);
  bootLeader();
}
...
async function bootLeader() {
  ...
  setLoginStatus("Memeriksa sesi leader...");
  const valid = await validateLeaderSession(hrToken);
  if (!valid) {
    clearLeaderSession();
    hrToken = "";
...
async function handleLeaderLogin(event) {
  ...
  setLoginStatus("Menghubungi server leader...");

  try {
    const form = new FormData();
    form.append("action", "leaderLogin");
    ...
    if (!payload?.status || !payload?.data?.token) {
      ...
      if (/unsupported post action/i.test(serverMessage)) {
        throw new Error("Backend Apps Script masih versi lama atau LeaderAuth.gs belum terpasang. Deploy ulang versi yang memiliki doPost() + handleLeaderAuthPost_().");
      }
      if (/belum dikonfigurasi/i.test(serverMessage)) {
        throw new Error("Login leader belum dikonfigurasi. Cek data akun di sheet Daftar Leader dan PPM_LEADER_TOKEN_SECRET di Script Properties.");
      }
      throw new Error(serverMessage);
    }

    // Token baru saja diterbitkan oleh backend yang menerima credential leader.
    // Jangan blokir login dengan request leaderValidate kedua yang redundan dan rawan gagal redirect/network.
    hrToken = String(payload.data.token);
    hrSessionVerified = true;
    saveLeaderSession({ token: hrToken, expiresAt: payload.data.expiresAt || "" });

    setLoginStatus("Login berhasil. Membuka dashboard...");
    openDashboardShell();
    passwordEl.value = "";

    // Data staff/progress dimuat SETELAH shell leader terbuka.
...
function openDashboardShell() {
  ...
  if (backendBannerEl) {
    ...
    backendBannerEl.textContent = "Login leader berhasil. Memuat data dashboard...";
  }
}

async function hydrateDashboardAfterLogin() {
  try {
    const data = await loadLeaderParticipants();
    participants = data;
    initializeDashboard();
  } catch (error) {
    console.error("Dashboard leader terbuka tetapi staff gagal dimuat:", error);
    participants = [];
    initializeDashboard();
    if (backendBannerEl) {
      backendBannerEl.hidden = false;
      backendBannerEl.textContent = `Akses leader berhasil, tetapi daftar staff gagal dimuat: ${formatLoginError(error)}`;
    }
  }
}
...
async function fetchJsonWithTimeout(url, options = {}, timeoutMs = 15000) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Server leader merespons HTTP ${response.status}.`);
    }
    if (!text.trim()) {
      throw new Error("Server leader mengembalikan respons kosong.");
    }
    try {
    ...
    if (error?.name === "AbortError") {
      throw new Error("Koneksi ke backend leader timeout. Periksa deployment Apps Script dan jaringan.");
    }
    if (error instanceof TypeError && /fetch/i.test(error.message || "")) {
      throw new Error("Browser tidak dapat membaca respons Apps Script (network/CORS/redirect). Pastikan Web App di-deploy untuk Anyone dan URL /exec di leader.js masih benar.");
    }
    throw error;
  } finally {
...
function formatLoginError(error) {
  ...
  return message || "Login gagal. Periksa akun leader dan deployment backend.";
}


async function validateLeaderSession(token) {
  try {
    const payload = await fetchJsonWithTimeout(
      `${HR_API}?action=leaderValidate&token=${encodeURIComponent(token)}&_=${Date.now()}`,
      { cache: "no-store", credentials: "omit", redirect: "follow" },
      12000
    );
    return Boolean(payload?.status && payload?.data?.valid);
  } catch (error) {
    console.warn("Validasi sesi leader gagal:", error);
    return false;
  }
}

async function loadLeaderParticipants() {
  const payload = await fetchJsonWithTimeout(
    `${HR_API}?action=leaderParticipants&token=${encodeURIComponent(hrToken)}&_=${Date.now()}`,
    { cache: "no-store", credentials: "omit", redirect: "follow" },
    15000
  );

  if (payload?.code === "UNAUTHORIZED") {
    const unauthorized = new Error("Sesi leader ditolak saat memuat daftar staff.");
    unauthorized.code = "UNAUTHORIZED";
    throw unauthorized;
  }

  if (!payload?.status || !Array.isArray(payload?.data)) {
    throw new Error(payload?.message || "Endpoint leaderParticipants tidak mengembalikan array staff.");
  }

  participantSource = "leaderParticipants";
...
function handleUnauthorized(context = "endpoint leader") {
  // Jangan reload otomatis. leaderLogin/leaderValidate sudah membuktikan sesi saat dashboard dibuka.
  // Jika endpoint tertentu menolak token, tampilkan diagnosis tanpa menghancurkan state UI.
  progressBackendReady = false;
  if (backendBannerEl) {
    backendBannerEl.hidden = false;
    backendBannerEl.textContent = `Dashboard tetap terbuka, tetapi ${context} menolak token leader. Cek deployment backend (LeaderAuth.gs + Progress.gs) dan pastikan semuanya berada pada versi deployment yang sama.`;
  }
  console.error(`${context} mengembalikan UNAUTHORIZED untuk sesi leader yang sebelumnya tervalidasi.`);
}

function readSavedSession() {
  try { return JSON.parse(sessionStorage.getItem(LEADER_SESSION_KEY) || "null"); }
  catch { return null; }
}
function saveLeaderSession(value) { sessionStorage.setItem(LEADER_SESSION_KEY, JSON.stringify(value)); }
...
function exportCSV() {
  ...
  a.href = url; a.download = `Leader-Monitoring-PPM-${new Date().toISOString().slice(0,10)}.csv`;
